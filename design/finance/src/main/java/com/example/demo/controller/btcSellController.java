...
@Controller
public class btcSellController {
    @Autowired
    ...
    public String btcbuyResult(Model model,@ModelAttribute trade trade2){
        ...
        if(count<=now_count) {
            ...
            btcbuyService.insertRecord(username,"btc",(0-count),newest_price,service_fee);
            result.setService_fee(service_fee);
            btcbuyService.subBTC(username, count);
        }
        else{
            System.out.println("您打算卖出的数量超过了持有量。");
            result.setStatus("交易失败");
            result.setComments("您打算卖出的比特币数量超过了持有量。");
        }
        model.addAttribute("result",result);
        return "btcbuyresult";
    }
