...
public class btcBuyController {
    ...
    public String btcbuyResult(Model model,@ModelAttribute trade trade2){
        ...
        if(new_balance>0) {
            ...
            btcbuyService.insertRecord(username,"btc",count,newest_price,service_fee);
            btcbuyService.addBTC(username, count);
            result.setService_fee(service_fee);
        }
        else{
            System.out.println("本金余额不足，无法完成购买。");
            result.setStatus("交易失败");
            result.setComments("本金余额不足，无法完成比特币购买。");
        }
        model.addAttribute("result",result);
        return "btcbuyresult";
    }
