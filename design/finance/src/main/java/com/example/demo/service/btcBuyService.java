...
public class btcBuyService {
    ...
    public List<btcprice> getBTCNewestPrice(){//获取美元汇率信息
        String sql="select PRICE from BTC_LTC_NEWEST_PRICE where TYPE='BTC'";
    ...
    public void setHolding(String username,float count){//更新用户持有量信息
        ...
        String insertSql = "update holding_"+username+" set BTC=" + balancestr +";";
        int rows = jdbcTemplate.update(insertSql);
    }

    public void addBTC(String username,float count){//增加用户当前持有的比特币
        //update holding_test set count=count+1 where code='btc';
        String countstr=String.valueOf(count);
        String insertSql="update holding_"+username+" set count=count+"+count+" where code='btc'";
        int rows = jdbcTemplate.update(insertSql);
    }

    public void subBTC(String username,float count){//增加用户当前持有的比特币
        //update holding_test set count=count+1 where code='btc';
        String countstr=String.valueOf(count);
        String insertSql="update holding_"+username+" set count=count-"+count+" where code='btc'";
        int rows = jdbcTemplate.update(insertSql);
    ...
    public List<btcprice> getHolding(String username){
        String sql="select count from holding_"+username+" where code='btc';";
        return(List<btcprice>) jdbcTemplate.query(sql, new RowMapper<btcprice>() {
            @Override
            public btcprice mapRow(ResultSet rs, int rowNum) throws SQLException {
